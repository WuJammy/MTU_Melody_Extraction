...
class PatchMerging(nn.Module):
    ...
    def __init__(self, input_resolution, dim, norm_layer=RMSNorm):
        super().__init__()
        self.input_resolution = input_resolution
        self.dim = dim
...
class PatchExpand(nn.Module):
    ...
    def __init__(self, input_resolution, dim, dim_scale=2, norm_layer=RMSNorm):
        super().__init__()
        self.input_resolution = input_resolution
        self.dim = dim
...
class FinalPatchExpand_X4(nn.Module):
    ...
    def __init__(self, input_resolution, dim, dim_scale=4, norm_layer=RMSNorm):
        super().__init__()
        self.input_resolution = input_resolution
        self.dim = dim
...
class MambaBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        #colum scan low to high input(front)
        b, n, c = x.shape
        input = x
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=[4,4], in_chans=3, embed_dim=96, norm_layer=None):
        super().__init__()
        img_size = to_2tuple(img_size)
        patch_size = patch_size
        stride_num = (patch_size[0]//2,patch_size[1]//2)
        # patches_resolution_h = (img_size[0] - patch_size[0] + stride_num)//stride_num
        # patches_resolution_w = (img_size[1] - patch_size[1] + stride_num)//stride_num
        # resize_patches_resolution_h = (patches_resolution_h%8)+patches_resolution_h
        # resize_patches_resolution_w = (patches_resolution_w%8)+patches_resolution_w
        ...
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=stride_num,)
    ...
    def forward(self, x):
        ...
        x = self.proj(
            x
        )  # -> (batch_size, self.embed_dim, self.img_size[0] / 4, self.img_size[0] / 4) = (24, 96, 56, 56)
        # b, c, h, w = x.shape
        # h = (h%8)+h
        # w = (w%8)+w
        x = F.interpolate(x, size=(56, 56), mode='bilinear', align_corners=False)
        #將以row進行flatten改成以column
        x = x.transpose(2,3)
        x = x.flatten(
            2
        )  # -> (batch_size, self.embed_dim, (self.img_size[0] / 4) * (self.img_size[0] / 4)) = (24, 96, 3136)
        x = x.transpose(1, 2)  # -> (24, 3136, 96)
...
class MambaTransformerSys(nn.Module):
    def __init__(self,
        ...
        print(
            "MambaTransformerSys expand initial----depths:{};depths_decoder:{};num_classes:{}"
            .format(depths, depths_decoder, num_classes))
        ...
        if self.final_upsample == "expand_first":
            ...
            self.up = FinalPatchExpand_X4(input_resolution=(img_size // 4,
                                                            img_size // 4),
                                          dim_scale=4,
                                          dim=embed_dim)
     

            self.output = ConvBlock(in_channels=embed_dim, out_channels=num_classes)
